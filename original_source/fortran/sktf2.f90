...
!  =======
!
!  SKTF2 computes the factorization of a skew-symmetric matrix A
...
!  Instead of a full tridiagonalization, SKTF2 can also compute a
...
!  =========
!
!     SUBROUTINE SKTF2( A, IPIV, UPLO, MODE, INFO)
...
!  ===============
!
!  The normal use for SKTF2 is to compute the U T U^T or L T L^T
!  decomposition of a skew-symmetric matrix with pivoting. This mode
!  is chosen by setting MODE = 'N' ("normal" mode). The other
!  use of SKTF2 is the computation the Pfaffian of a skew-symmetric matrix,
...
!  or in the even columns (if UPLU = 'U') are properly computed by SKTF2.
!
...
SUBROUTINE SSKTF2_F95( A, IPIV, UPLO, MODE, INFO )
  ...
  ELSE IF( .NOT.LSAME(LMODE,'N') .AND. .NOT.LSAME(LMODE,'P') ) THEN
     LINFO = -4
  ELSE IF( N>0 ) THEN
     CALL SKTF2( LUPLO, LMODE, N, A, LDA, IPIV, LINFO )
  END IF

  CALL MESSAGE(LINFO, "SKTF2", INFO)

      SUBROUTINE CLASKTRF( UPLO, MODE, N, NB, A, LDA, IPIV, W, LDW,
     $                     INFO)
*
*     .. Scalar Arguments ..
      CHARACTER          UPLO, MODE
...
*  =======
*
*  CLASKTRF computes NB steps of the factorization of a skew-symmetric
*  matrix A using the Parlett-Reid algorithm:
*
*     P*A*P^T = U*T*U^T  or  P*A*P^T = L*T*L^T
*
*  If UPLO = 'U', CLASKTRF reduces the last NB rows and columns of a
*  matrix, of which the upper triangle is supplied;
*  if UPLO = 'L', CLASKTRF reduces the first NB rows and columns of a
...
*     .. External Subroutines ..
      EXTERNAL           CSCAL, CSWAP, CSKR2K, CGEMV, CCOPY, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
            ...
            IF( K .LT. N) THEN
               ...
               IF( WK .GT. 0 ) THEN
                  A( K, K ) = ZERO
                  CALL CGEMV( 'N', K, WK, +ONE, A( 1, N-(WK-1)*STEP ),
     $                 LDA*STEP, W( K, NB-WK+1 ), LDW, ONE,
     $                 A( 1, K ), 1 )
                  CALL CGEMV( 'N', K, WK, -ONE, W( 1, NB-WK+1 ),
...
*     Store the (updated) column K in W(:,NB-WK+1)
               IF( MOD(K, STEP).EQ.1 .OR. STEP.EQ.1 ) THEN
                  WK = WK + 1
                  CALL CCOPY(K, A(1, K), 1, W(1, NB-WK+1 ), 1)
               END IF
            END IF
...
*     Also, swap the first K-1 columns of W
               ...
               IF( KP .NE. KK ) THEN
                  ...
                  CALL CSWAP( KK-KP-1, A( KP+1, KK ), 1,
     $                 A( KP, KP+1 ), LDA )

                  CALL CSWAP( N-K+1, A( KK, K), LDA, A( KP, K), LDA)

                  CALL CSCAL(KK-KP, -ONE, A(KP, KK), 1)
                  CALL CSCAL(KK-KP-1, -ONE, A(KP, KP+1), LDA)

*     Swap in the last columns of W
                  IF( WK .GT. 0 ) THEN
                     CALL CSWAP( WK, W( KK, NB-WK+1 ), LDW,
     $                    W( KP, NB-WK+1 ), LDW)
                  END IF
               END IF
...
*     Store L(k+1) in A(k)
                  CALL CSCAL(K-2, ONE/A( K-1, K ), A(1, K), 1)
               END IF
...
*     Store the column N-NB in W, and update it in W
               CALL CCOPY(N-NPANEL, A(1, N-NPANEL), 1,
     $              W(1, 1), 1)

               W( N-NPANEL, 1 ) = ZERO
               CALL CGEMV( 'N', N-NPANEL, WK, +ONE,
     $              A( 1, N-(WK-1)*STEP ), LDA*STEP,
     $              W( N-NPANEL, NB-WK+1 ), LDW, ONE, W( 1, 1 ), 1 )
               CALL CGEMV( 'N', N-NPANEL, WK, -ONE, W( 1, NB-WK+1 ),
...
*     Now do the rank-3 update
            CALL CSKR2K( UPLO, "N", N-NPANEL, NB, ONE,
     $           A(1, N-(WK-1)*STEP), LDA*STEP,
...
*
            ...
            IF( K .GT. 1) THEN
               ...
               IF( WK .GT. 0) THEN
                  A( K, K ) = ZERO
                  CALL CGEMV( 'N', N-K+1, WK, +ONE, A( K, 1 ),
     $                 LDA*STEP, W( K, 1 ), LDW, ONE, A( K, K ), 1 )
                  CALL CGEMV( 'N', N-K+1, WK, -ONE, W( K, 1 ),
...
*     Store the (updated) column K in W(:,WK)
               IF( MOD(K, STEP) . EQ. 0 ) THEN
                  WK = WK + 1
                  CALL CCOPY(N-K+1, A(K, K), 1, W(K, WK), 1)
               END IF
            END IF
...
*     Also, swap the first K-1 columns of W
               ...
               IF( KP .NE. KK ) THEN
                  IF( KP.LT.N ) THEN
                     CALL CSWAP( N-KP, A( KP+1, KK ), 1,
     $                           A( KP+1, KP ),1 )
                  END IF

                  CALL CSWAP( KP-KK-1, A( KK+1, KK ), 1,
     $                        A( KP, KK+1 ), LDA )

                  CALL CSWAP( K, A( KK, 1), LDA, A( KP, 1), LDA)

*     The matrix is anti-symmetric, hence swaps beyond the diagonal need a -1

                  CALL CSCAL( KP-KK, -ONE, A(KK+1, KK), 1 )
                  CALL CSCAL( KP-KK-1, -ONE, A(KP, KK+1), LDA )

*     Swap in the first columns of W
                  CALL CSWAP( WK, W( KK, 1 ), LDW, W( KP, 1 ), LDW)
               END IF

*     (The column/row K+1 is not affected by the update)
               IF( COLMAX .NE. ZERO .AND. K .LE. N-2) THEN
*     Store L(k+1) in A(k)
                  CALL CSCAL( N-K-1, ONE/A( K+1, K ), A(K+2, K), 1 )
               END IF
...
*     Store the column NB+1 in W, and update it in W
               CALL CCOPY(N-NPANEL, A(NPANEL+1, NPANEL+1), 1,
     $              W(NPANEL+1, NB), 1)

               W( NPANEL+1, NB ) = ZERO
               CALL CGEMV( 'N', N-NPANEL, NB-1, +ONE, A( NPANEL+1, 1 ),
     $              LDA*STEP, W( NPANEL+1, 1 ), LDW,
     $              ONE, W( NPANEL+1, NB ), 1 )
               CALL CGEMV( 'N', N-NPANEL, NB-1, -ONE, W( NPANEL+1, 1 ),

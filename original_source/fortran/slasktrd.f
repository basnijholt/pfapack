...
*
               A( I, I ) = ZERO
               CALL SGEMV( 'No transpose', I, NW2, +ONE,
...
*
               ...
               CALL SSKMV( 'Upper', I-1, TAU(I-1), A, LDA,
     $                     A( 1, I ), 1, ZERO, W( 1, NB-NW ), 1 )
               IF( NW .GT. 0 ) THEN
                  CALL SGEMV( 'Transpose', I-1, NW, ONE,
     $                        W( 1, NB-NW+1 ), LDW, A( 1, I ), 1, ZERO,
     $                        W( I+1, NB-NW ), 1 )
                  CALL SGEMV( 'No transpose', I-1, NW, TAU(I-1),
...
*
            ...
            IF( NW2 .GT. 0 ) THEN
               A( I, I ) = ZERO
               CALL SGEMV( 'No transpose', N-I+1, NW2, +ONE, A( I, 1 ),
     $                     LDA*STEP, W( I, 1 ), LDW, ONE, A( I, I ), 1 )
               CALL SGEMV( 'No transpose', N-I+1, NW2, -ONE, W( I, 1 ),
...
*
               ALPHA = A( I+1, I )
               CALL SLARFG( N-I, ALPHA, A( MIN( I+2, N ), I ), 1,
     $                      TAU( I ) )
               E( I ) = ALPHA
               A( I+1, I ) = ONE
*
...
*              This is given by tau A^(i)*v^*=tau(A*v^* + VW^T v^* - WV^T v^*)
               ...
               CALL SSKMV( 'Lower', N-I, TAU( I ),
     $                     A( I+1, I+1 ), LDA,
     $                     A( I+1, I ), 1, ZERO, W( I+1, NW+1 ), 1 )
               IF( NW .GT. 0 ) THEN
                  CALL SGEMV( 'Transpose', N-I, NW, ONE,
     $                 W( I+1, 1 ), LDW, A( I+1, I ), 1, ZERO,
     $                 W( 1, NW+1 ), 1 )
                  CALL SGEMV( 'No transpose', N-I, NW, TAU( I ),

...
!  =========
!
!     SUBROUTINE SKBPF10( AB, PFAFF, UPLO, INFO)
...
SUBROUTINE SSKBPF10_F95( AB, PFAFF, UPLO, INFO )
  ...
  ELSE IF( N>0 ) THEN
     ...
     IF( ISTAT == 0) THEN
        CALL SKBPF10( LUPLO, N, KD, AB, LDAB, PFAFF, WORK, LINFO )
     ELSE
  ...
  CALL MESSAGE(LINFO, 'SKBPF10', INFO, ISTAT)
...
SUBROUTINE CSKBPF10_F95( AB, PFAFF, UPLO, INFO )
  ...
  ELSE IF( N>0 ) THEN
     ...
     IF( ISTAT == 0) THEN
        CALL SKBPF10( LUPLO, N, KD, AB, LDAB, PFAFF, WORK, RWORK, LINFO )
     ELSE
        LINFO=-100 ! not enough memory
     END IF

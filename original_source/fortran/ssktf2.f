...
*     .. External Subroutines ..
      EXTERNAL           SSCAL, SSWAP, SSKR2, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX
*     ..
...
*     full sub-matrix A(1:N,1:N)
               ...
               IF( KP .NE. KK ) THEN
                  ...
                  CALL SSWAP( KK-KP-1, A( KP+1, KK ), 1,
     $                 A( KP, KP+1 ), LDA )
...
*     (The column/row K-1 is not affected by the update)
               IF( COLMAX .NE. ZERO ) THEN
                  CALL SSKR2( UPLO, K-2, ONE/A( K-1,K ), A( 1, K ), 1,
     $                 A( 1, K-1 ), 1, A( 1, 1 ), LDA )
...
*     full matrix A(1:N,1:N)
               ...
               IF( KP .NE. KK ) THEN
                  IF( KP.LT.N ) THEN
                     CALL SSWAP( N-KP, A( KP+1, KK ), 1,
     $                    A( KP+1, KP ),1 )
                  END IF

                  CALL SSWAP( KP-KK-1, A( KK+1, KK ), 1,
     $                 A( KP, KK+1 ), LDA )
...
*     (The column/row K+1 is not affected by the update)
               IF( COLMAX .NE. ZERO .AND. K+2 .LE. N) THEN
                  CALL SSKR2( UPLO, N-K-1, ONE/A( K+1,K ),

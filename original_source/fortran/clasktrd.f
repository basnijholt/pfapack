      SUBROUTINE CLASKTRD( UPLO, MODE, N, NB, A, LDA, E, TAU, W, LDW )
*
...
*  =======
*
*  CLASKTRD reduces NB rows and columns of a complex skew-symmetric matrix A to
*  skew-symmetric tridiagonal form by a unitary congruence
*  transformation Q^H * A * Q^*, and returns the matrices V and W which are
*  needed to apply the transformation to the unreduced part of A.
*
*  If UPLO = 'U', CLASKTRD reduces the last NB rows and columns of a
*  matrix, of which the upper triangle is supplied;
*  if UPLO = 'L', CLASKTRD reduces the first NB rows and columns of a
*  matrix, of which the lower triangle is supplied.
*
*  Alternatively, the routine can also be used to compute a partial
*  tridiagonal form
*
*  This is an auxiliary routine called by CSKTRD.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           CGEMV, CSKMV, CLACGV, CLARFG
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*
               A( I, I ) = ZERO
               CALL CGEMV( 'No transpose', I, NW2, +ONE,
...
*              We need to multiply with v^*
               CALL CLACGV( I-1, A( 1, I ), 1)

               CALL CSKMV( 'Upper', I-1, CONJG(TAU(I-1)), A, LDA,
     $                     A( 1, I ), 1, ZERO, W( 1, NB-NW ), 1 )
               IF( NW .GT. 0 ) THEN
                  CALL CGEMV( 'Transpose', I-1, NW, ONE,
     $                        W( 1, NB-NW+1 ), LDW, A( 1, I ), 1, ZERO,
     $                        W( I+1, NB-NW ), 1 )
                  CALL CGEMV( 'No transpose', I-1, NW, CONJG(TAU(I-1)),
     ...
     $                        A( 1, I ), 1, ZERO,
     $                        W( I+1, NB-NW ), 1 )
                  CALL CGEMV( 'No transpose', I-1, NW,
     $                        -CONJG(TAU(I-1)),
     $                        W( 1, NB-NW+1 ), LDW,
     $                        W( I+1, NB-NW ), 1,
     $                        ONE, W( 1, NB-NW ), 1 )
...
*
            ...
            IF( NW2 .GT. 0 ) THEN
               A( I, I ) = ZERO
               CALL CGEMV( 'No transpose', N-I+1, NW2, +ONE, A( I, 1 ),
     $                     LDA*STEP, W( I, 1 ), LDW, ONE, A( I, I ), 1 )
               CALL CGEMV( 'No transpose', N-I+1, NW2, -ONE, W( I, 1 ),
...
*              complex conjugate of v
               CALL CLACGV( N-I, A( I+1, I ), 1 )

               CALL CSKMV( 'Lower', N-I, CONJG(TAU( I )),
     $                     A( I+1, I+1 ), LDA,
     $                     A( I+1, I ), 1, ZERO, W( I+1, NW+1 ), 1 )
               IF( NW .GT. 0 ) THEN
                  CALL CGEMV( 'Transpose', N-I, NW, ONE,
     $                 W( I+1, 1 ), LDW, A( I+1, I ), 1, ZERO,
     $                 W( 1, NW+1 ), 1 )
                  CALL CGEMV( 'No transpose', N-I, NW, CONJG(TAU( I )),

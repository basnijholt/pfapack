...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, SSKMV, SSKR2, SLARFG
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*              Compute  x := tau * A * v  storing x in TAU(1:i)
               ...
               CALL SSKMV( UPLO, I, TAUI, A, LDA,
     $                     A( 1, I+1 ),1, ZERO, TAU, 1 )
*
*              Apply the transformation as a rank-2 update:
*                 A := A + v * w^T - w * v^T
*
               CALL SSKR2( UPLO, I-STEP+1, ONE, A( 1, I+1 ), 1, TAU, 1,
     $                     A, LDA )
*
            ELSE
               A( I, I ) = ZERO
...
*
               CALL SSKMV( UPLO, N-I, TAUI, A( I+1, I+1 ), LDA,
     $                     A( I+1, I ), 1, ZERO, TAU( I ), 1 )
*
*              Apply the transformation as a rank-2 update:
*                 A := A + v * x^T - x * v^T
*
               CALL SSKR2( UPLO, N-I-STEP+1, ONE, A( I+STEP, I ), 1,
     $                     TAU( I+STEP-1 ), 1,
     $                     A( I+STEP, I+STEP ), LDA )
*
            ELSE

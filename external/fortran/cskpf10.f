...
*  =======
*
*  CSKPF10 computes the Pfaffian of a complex skew-symmetric matrix, taking
...
*
*  RWORK   (workspace) REAL array, dimension (N-1)
...
*  Q^H * A * Q^* = pT (MTHD = 'H').
*  These transformations are computed by the routines CSKTRF or CSKTRD,
...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, CSKTRD, CSKTRF, CMUL10
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
     ...
     $         LWORK.LT.N .AND. .NOT.LQUERY ) THEN
      ...
      IF( INFO.EQ.0 .AND. LQUERY) THEN
         IF( MOD(N,2).EQ.1 ) THEN
            WORK(1) = 1
         ELSE IF( LTL ) THEN
*     Defer workspace query to CSKTRF
            CALL CSKTRF( UPLO, "P", N, A, LDA, IWORK, WORK, LWORK,
     $                   INFO )
         ELSE
*     Defer workspace query to CSKTRD
            CALL CSKTRD( UPLO, "P", N, A, LDA, RWORK, WORK, WORK,
     $                   LWORK, INFO)
            WORK(1) = WORK(1) + N - 1
         END IF
      END IF
...
*     Compute tridiagonal form
         CALL CSKTRF( UPLO, "P", N, A, LDA, IWORK, WORK, LWORK, INFO )
...
*     Reduce to tridiagonal form
         CALL CSKTRD(UPLO, "P", N, A, LDA, RWORK, WORK,

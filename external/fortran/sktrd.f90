...
!  =======
!
!  SKTRD reduces a real or complex skew-symmetric matrix A to real
...
!  =========
!
!     SUBROUTINE SKTRD( A, TAU, UPLO, MODE, INFO)
...
SUBROUTINE SSKTRD_F95( A, TAU, UPLO, MODE, INFO )
  ...
  ELSE IF( N>0 ) THEN
     ...
     IF( ISTAT == 0) THEN
        ...
        CALL SKTRD( LUPLO, LMODE, N, A, LDA, E, TAU, QWORK, -1, LINFO)
        LWORK = INT(QWORK(1))
        ...
        IF( ISTAT /= 0 ) THEN
           ...
           DEALLOCATE(WORK)
           CALL MESSAGE(-200, 'SKTRD') ! print warning about not enough workspace
     ...
     IF( ISTAT == 0 ) THEN
        CALL SKTRD( LUPLO, LMODE, N, A, LDA, E, TAU, WORK, LWORK, LINFO )
     ELSE
  ...
  CALL MESSAGE(LINFO, 'SKTRD', INFO, ISTAT)

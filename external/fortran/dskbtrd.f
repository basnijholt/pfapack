...
*
*  -- Based on LAPACK routine ZHBTRD (version 3.2) --
...
*     ..
*     .. Array Arguments ..
      DOUBLE PRECISION   E( * ), AB( LDAB, * ), Q( LDQ, * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  DSKBTRD reduces a real skew-symmetric band matrix A to
*  skew-symmetric tridiagonal form T by an orthogonal similarity
*  transformation: Q^T * A * Q = T. Alternatively, the routine can
*  also compute a partial tridiagonal form useful for computing the Pfaffian.
*  This routine only uses orthogonal transformations with det = 1.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, DLARGV, DLARTG, DLARTV,
     $                   DLASET, DROT
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
*     The real cosines and sines of the plane rotations are
*     stored in the array WORK (Note: the symmetric version in LAPACK
*     has an array D [diagonal] and WORK, Here we only work with a single
*     array WORK of twice the size).
*
      INCA = KD1*LDAB
      KDN = MIN( N-1, KD )
      IF( UPPER ) THEN
*
         IF( KD.GT.1 ) THEN
*
*           Reduce to skew-symmetric tridiagonal form, working with
*           the upper triangle
*
            NR = 0
            J1 = KDN + 2

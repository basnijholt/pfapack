...
!  =======
!
!  SKPFA computes the Pfaffian of a dense skew-symmetric matrix.
!
!  =========
!
!     SUBROUTINE SKPFA( A, PFAFF, UPLO, MTHD, INFO)
...
SUBROUTINE SSKPFA_F95( A, PFAFF, UPLO, MTHD, INFO)
  ...
  ELSE IF( N>0 ) THEN
     ...
     IF( ISTAT == 0 ) THEN
        ...
        CALL SKPFA( LUPLO, LMTHD, N, A, LDA, PFAFF, IWORK, QWORK, -1, LINFO)
        LWORK = INT(QWORK(1))
        ...
        IF( ISTAT /= 0 ) THEN
           ...
           DEALLOCATE(WORK)
           CALL MESSAGE(-200, 'SKPFA') ! print warning about not enough wspace
     ...
     IF( ISTAT == 0 ) THEN
        CALL SKPFA( LUPLO, LMTHD, N, A, LDA, PFAFF, IWORK, WORK, LWORK, LINFO )
     ELSE
  ...
  CALL MESSAGE(LINFO, 'SKPFA', INFO, ISTAT)
...
SUBROUTINE CSKPFA_F95( A, PFAFF, UPLO, MTHD, INFO)
  ...
  ELSE IF( N>0 ) THEN
     ...
     IF( ISTAT == 0 ) THEN
        !Do a workspace query
        CALL SKPFA( LUPLO, LMTHD, N, A, LDA, PFAFF, IWORK, QWORK, -1, &
                    RWORK, LINFO)
        LWORK = INT(QWORK(1))
        ALLOCATE(WORK(LWORK), STAT=ISTAT)
     ...
     IF( ISTAT == 0 ) THEN
        CALL SKPFA( LUPLO, LMTHD, N, A, LDA, PFAFF, IWORK, WORK, LWORK, &
                    RWORK, LINFO )
     ELSE
        LINFO=-100 ! not enough memory
     END IF
